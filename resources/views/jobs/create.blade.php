...
/* ── Summary card — sticky mobile, fixed desktop ────── */
.summary-card {
    border: none; border-radius: 14px; background: #ffffff;
...
<div class="col-lg-8 d-flex flex-column gap-4">
    ...
    <div class="job-form-card card">
        <div class="card-body p-4">
            ...
            {{-- Customer info preview strip (shown when customer selected) --}}
            <div id="customer-preview" class="mt-3 p-3 rounded-3" style="display:none;background:var(--bs-tertiary-bg);border:1px solid var(--bs-border-color);">
                <div class="d-flex align-items-center gap-3">
                    <div id="customer-avatar" style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#0d6efd,#0099ff);display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;color:#fff;flex-shrink:0;"></div>
                    <div class="flex-grow-1">
                        <div class="fw-bold" id="preview-name" style="font-size:14px;"></div>
                        <div class="d-flex flex-wrap gap-3 mt-1">
                            <span id="preview-phone" class="text-secondary small" style="display:none;"><i class="bi bi-telephone me-1"></i><span></span></span>
                            <span id="preview-email" class="text-secondary small" style="display:none;"><i class="bi bi-envelope me-1"></i><span></span></span>
                            <span id="preview-address" class="text-secondary small" style="display:none;"><i class="bi bi-geo-alt me-1"></i><span></span></span>
                        </div>
                    </div>
                    <button type="button" onclick="openEditCustomerModal()" class="btn btn-sm btn-outline-secondary flex-shrink-0">
...
function onCustomerChange(sel, overrideData) {
    ...
    // Always keep global in sync so filterVoucherList works reliably
    ...
    // Find option by value (reliable with Tom Select)
    const opt = sel.querySelector(`option[value="${id}"]`);

    // Use override data (passed after save) or fall back to option attributes
