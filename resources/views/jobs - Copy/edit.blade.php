    ...
    <div class="job-form-card card">
        <div class="card-body p-4">
            ...
            <div class="row g-3">
                ...
                <div class="col-sm-4"><label class="form-label">Completion Date</label><input type="date" class="form-control" name="date_out" value="{{ $job->date_out?->format('Y-m-d') }}"></div>
...
<div class="summary-card card" id="summary-card">
    <div class="card-body p-4">
        ...
        <div class="text-center pb-3 mb-3 border-bottom">
            ...
            <div class="summary-total text-primary" id="summary-total">£{{ number_format($job->totalAfterDiscount(),2) }}</div>
        </div>
        <div class="d-flex flex-column gap-2 mb-3">
            <div class="d-flex justify-content-between align-items-center">
                <span class="text-secondary small">Subtotal</span>
                <span class="fw-semibold" id="summary-subtotal">£{{ number_format($job->devices->sum(fn($d) => $d->repairItems->sum('price')),2) }}</span>
            </div>
            <div class="applied-row" id="row-discount" style="{{ $job->discount_type ? 'display:flex;' : 'display:none;' }}">
                <span class="text-secondary small" id="lbl-discount">{{ $job->discount_type === 'percent' ? 'Discount ('.$job->discount_value.'%)' : 'Discount (Fixed)' }}</span>
                <span class="fw-semibold text-danger" id="val-discount">-£{{ number_format($job->discountAmount(),2) }}</span>
            </div>
            <div class="applied-row" id="row-voucher" style="{{ $job->voucher_code ? 'display:flex;' : 'display:none;' }}">
                <span class="text-secondary small" id="lbl-voucher">🎟️ {{ $job->voucher_code }}</span>
                <span class="fw-semibold text-success" id="val-voucher">-£{{ number_format($job->voucher_amount ?? 0,2) }}</span>
            </div>
            @if($job->totalPaid() > 0)
            <div class="d-flex justify-content-between align-items-center p-2 rounded-2" style="background:rgba(25,135,84,.06);">
                ...
                <span class="fw-semibold text-success" id="total-paid-display">£{{ number_format($job->totalPaid(),2) }}</span>
            </div>
            @endif
        </div>

        {{-- Balance Due — dynamic, updated by recalc() --}}
        <div id="balance-box-due" class="d-flex justify-content-between align-items-center p-3 mb-3 rounded-3" style="background:rgba(220,53,69,.08);border:1px solid rgba(220,53,69,.2);{{ $job->isPaidInFull() ? 'display:none!important;' : '' }}">
            <span class="fw-bold">Balance Due</span>
            <span class="fw-bold text-danger" style="font-size:20px;font-family:'Syne',sans-serif;" id="balance-due-amount">£{{ number_format($job->balanceDue(),2) }}</span>
        </div>
        <div id="balance-box-paid" class="text-center py-2 mb-3 rounded-3 fw-bold" style="background:rgba(25,135,84,.1);border:2px solid rgba(25,135,84,.3);color:#198754;font-size:15px;{{ $job->isPaidInFull() ? '' : 'display:none;' }}">✅ Fully Paid</div>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius:18px;overflow:hidden;">
            ...
            <div class="modal-body px-4 pt-3 pb-2">
                ...
                <div class="rounded-3 p-3" style="background:rgba(25,135,84,.05);border:1.5px solid rgba(25,135,84,.15);">
                    ...
                    {{-- Always-present hidden field — pre-filled if voucher exists --}}
                    <input type="hidden" id="voucher_code_hidden" name="voucher_code"
                        value="{{ $job->voucher_code ?? '' }}"
                        data-amount="{{ $job->voucher_amount ?? 0 }}">

                    {{-- Applied voucher strip — shown if voucher exists, hidden if not --}}
                    <div id="edit-applied-voucher"
                        class="d-flex align-items-center justify-content-between p-2 mb-2 rounded-2"
                        style="background:rgba(25,135,84,.12);border:1px solid rgba(25,135,84,.25);{{ $job->voucher_code ? '' : 'display:none;' }}">
                        <span class="text-success small fw-semibold" id="edit-applied-voucher-label">
                            ✅ <strong>{{ $job->voucher_code ?? '' }}</strong>@if($job->voucher_code) — -£{{ number_format($job->voucher_amount ?? 0, 2) }}@endif
                        </span>
